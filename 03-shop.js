...
containerEl.insertAdjacentHTML("beforeend", createMarkup(instruments));
...
function handleClick(event) {
  ...
  const perentEl = event.target.closest(".js-product");
  const productId = Number(perentEl.dataset.id);
  const currentProduct = instruments.find(({ id }) => id === productId);

  const productsArr = JSON.parse(localStorage.getItem(PRODUCT_LS_KEY)) ?? [];
  const index = productsArr.findIndex(({ id }) => id === productId);

  if (index === -1) {
    currentProduct.qty = 1;
    productsArr.push(currentProduct);
  } else {
    productsArr[index].qty += 1;
  }

  localStorage.setItem(PRODUCT_LS_KEY, JSON.stringify(productsArr));
}
